...
# turksiberguvenllik.net
# Coded By [TSG-TIM]R3D
import urllib2
import sys
import threading
import random
...
class MonitorThread(threading.Thread):
	def run(self):
		...
		while flag==0:
			if (previous+3<request_counter) & (previous<>request_counter):
				print ">>>Gonderilen %d<<<" % (request_counter)
				previous=request_counter
		if flag==2:
			print ">>>>Saldiri Kesildi<<<<"
